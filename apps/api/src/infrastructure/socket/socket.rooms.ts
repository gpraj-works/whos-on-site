...
export function joinCompanyRoom(socket: Socket): void {
  ...
  if (!authData || !authData.companyId) {
    logger.warn({ socketId: socket.id }, 'Attempted room join without verified companyId in socket data')
    return
  }
  ...
  logger.info({ socketId: socket.id, companyId: authData.companyId, roomName }, 'Socket client joined company room')
}
