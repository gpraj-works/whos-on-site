...
import { companies, customers, jobs, users, teamMembers } from '../schema/index'
    ...
    },
    {
      ...
      assignedTeamMemberId: (
        await db
          ...
          .from(teamMembers)
          .where(sql`${teamMembers.name} = 'John Atlanta Tech'`)
      )[0]?.id,
    ...
    },
    {
      ...
      assignedTeamMemberId: (
        await db
          .select({ id: teamMembers.id })
          .from(teamMembers)
          .where(sql`${teamMembers.name} = 'Mike Brooklyn Tech'`)
      )[0]?.id,
