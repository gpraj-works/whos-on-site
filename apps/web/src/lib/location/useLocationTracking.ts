...
import { updateTeamMemberLocation } from '../../components/team/api'
import { getSocket } from '../socket/client'

const MIN_PING_INTERVAL_MS = 10000 // 10 seconds minimum throttle

export function useLocationTracking(teamMemberId: string | null, enabled: boolean): void {
  const lastPingTimeRef = useRef<number>(0)

  useEffect(() => {
    if (!enabled || !teamMemberId || !('geolocation' in navigator)) {
      return
    }
    ...
    const handleSuccess = (position: GeolocationPosition) => {
      ...
      updateTeamMemberLocation(teamMemberId, { lat, lng }).catch((err) => {
