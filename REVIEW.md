...
Review ONLY changes introduced by the CURRENT checked-out branch compared with the target base branch (default: `origin/main`).
...
If `git fetch origin main` fails, report the limitation and use the best available `origin/main` reference. Do not claim it is current.
...
Determine whether this branch is safe to merge into `main`.
